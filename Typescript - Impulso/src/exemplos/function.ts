...
function somarValoresNumericosETratar(number1: number, number2: number, callback: (number3: number) => number):number {
    let resultado = number1 + number2;
    return callback(resultado);
}

function aoQuadrado(number3: number): number {
    return number3*number3;
}

function dividirPorEleMesmo(number3: number) {
    return number3 / number3;
}
